...
class VoiceEngine:
    ...
    def speak(self, text: str):
        """Speak text locally via pyttsx3 or printer fallback"""
    ...
    def _listen_worker(self):
        ...
        try:
            ...
            print("[Voice] PyAudio not available. Voice hearing disabled.")
            self.has_stt_active = False
        except Exception as e:
    ...
    def get_dependencies_status(self) -> str:
        ...
        status.append(f"STT (vosk/pyaudio/sounddevice): {'INSTALLED' if HAS_STT else 'PARTIAL (sounddevice active)'}")
        
        if not (HAS_TTS and HAS_STT):
            status.append("\nTo enable Full Voice, run:")
            status.append("pip install pyttsx3 vosk sounddevice")
